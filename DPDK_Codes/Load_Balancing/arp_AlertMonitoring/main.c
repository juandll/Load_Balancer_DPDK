...
static int alm_v=0;
static uint16_t arp_lb;
static uint8_t arp_r;
...
{ //checking destination is the servers
	if(dest_portid == 1){
		...
		arp_lb = eth->ether_type;
		arp_pr = rte_pktmbuf_mtod_offset(m, uint8_t *,21);
		arp_r = *arp_pr;
		//checking protocolo, we readed the protocol by big-end so the normal protocol is 08x06, but we read it 06x08
		if((arp_lb == 0x0608) && (arp_r == 0x01)){
			// save mac adress into the buffer to change the packet
			tmp = &eth->d_addr.addr_bytes[0];
			*((uint64_t *)tmp) = destino_server[alm_v];
			rte_ether_addr_copy(&lb_ports_eth_addr[dest_portid], &eth->s_addr);
		}
    ...
    eth = rte_pktmbuf_mtod(m, struct rte_ether_hdr *);
		...
		if(count_1==0){
			for(int j=5;j>=0;j--){
				lba_aux=eth->s_addr.addr_bytes[j];// we save the whole mac on a variable
			...
			if(src_mac_lb == destino_server[alm_v]){//we change alert monitoring varialbe if the server who sent the alert is the one in use
				alm_v++;
				if(alm_v>2){
					alm_v=0;
				}
