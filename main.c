...
l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid)
{
	if(dest_portid == 1){
		...
		uint8_t des_mac_alert[6] = {0x02,0x00,0x00,0x00,0x00,0x01};
		uint8_t lba_aux;
		uint8_t num_act=0;
                eth = rte_pktmbuf_mtod(m, struct rte_ether_hdr *);
		for(int i=0;i<6;i++){
		...
		if(count_1==0){
			...
			for(int z=0;z<server_number;z++){
				if(src_mac_lb == destino_server[z]){
					num_act =*rte_pktmbuf_mtod_offset(m, uint8_t *,42);
					printf("\n Valor de %d: %d",z,num_act);

					if(min_num[z] == 101){
						min_num[z] = num_act;
						server_count++;
						printf("\n Valor de %d: %d",z,min_num[z]);
					}
				}
			}
			uint8_t local_min = 101;
			uint8_t min_index;
			if(server_count > (server_number-1)){
				server_count = 0;
